  ...
  const next = async () => {
    ...
    try {
      ...
      toast({
        title: "Load Posted! ✅",
        description:
          "Your load is now visible to drivers (आपका लोड ड्राइवरों को दिख रहा है)",
      });
          ...
          >
            ...
            {step === 0 && (
              <div className="space-y-3">
                <SelectField
                  ...
                  onChange={(v) => update("pickupState", v)}
                ...
                {data.pickupState && (
                  <SelectField
                    ...
                    options={
                      MAJOR_CITIES[data.pickupState] || ["Other"]
                    }
                  />
                )}
              </div>
            ...
            {step === 1 && (
              <div className="space-y-3">
                <SelectField
                  label="State"
                  value={data.dropState}
                  onChange={(v) => update("dropState", v)}
                  options={INDIAN_STATES as unknown as string[]}
                />
                {data.dropState && (
                  <SelectField
                    label="City"
                    value={data.dropCity}
                    onChange={(v) => update("dropCity", v)}
                    options={
                      MAJOR_CITIES[data.dropState] || ["Other"]
                    }
                  />
                )}
              </div>
        ...
        <div className="flex gap-3 mt-6">
          {step > 0 && (
            <button
              onClick={back}
              disabled={loading}
              className="btn-glass py-3 px-5 rounded-xl flex items-center gap-2 disabled:opacity-50"
            >
